  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
    ...
    <div class="bg-gray-900 rounded-xl p-4 border border-gray-800">
      <div class="flex items-center justify-between mb-3">
        ...
        <button @click="startNew" class="text-xs bg-indigo-700 hover:bg-indigo-600 px-2 py-1 rounded transition-colors">+ 新建</button>
      </div>
      ...
      <div v-for="tpl in templates" :key="tpl.name"
        ...
        <div class="flex items-center justify-between">
          <span class="font-mono text-sm text-indigo-300">{{ tpl.name }}</span>
          <button @click.stop="confirmDelete(tpl.name)"
            class="text-xs text-red-500 hover:text-red-400 opacity-0 group-hover:opacity-100">删除</button>
        </div>
    ...
    <!-- editor -->
    <div class="lg:col-span-2 bg-gray-900 rounded-xl p-4 border border-gray-800">
      <template v-if="editing">
        <div class="flex items-center gap-3 mb-4">
          <input v-model="editing.name" :disabled="!isNew"
            placeholder="task_type 名称 (如 AFK)"
            class="bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5 text-sm font-mono focus:outline-none focus:border-indigo-500 disabled:opacity-60 w-40" />
          <input v-model="editing.description" placeholder="描述（可选）"
            class="flex-1 bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-indigo-500" />
          <div class="flex items-center gap-1">
            <label class="text-xs text-gray-500">默认 task_time(s)</label>
            <input v-model.number="editing.task_time" type="number"
              class="w-20 bg-gray-800 border border-gray-700 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:border-indigo-500" />
          </div>
        </div>
        ...
        <div class="flex items-center gap-3">
          <button @click="save" :disabled="saving"
            class="bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 px-4 py-1.5 rounded-lg text-sm font-medium transition-colors">
            {{ saving ? '保存中...' : '保存' }}
          </button>
          <button @click="editing = null" class="text-xs text-gray-500 hover:text-gray-300">取消</button>
          <span v-if="saveMsg" :class="saveError ? 'text-red-400' : 'text-green-400'" class="text-xs">{{ saveMsg }}</span>
        </div>
      </template>
