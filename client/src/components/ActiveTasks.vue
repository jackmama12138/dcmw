    ...
    <div class="at__body">
      ...
      <div v-for="group in urlGroups" :key="group.url" class="at__row">
        ...
        <a-space :size="4" class="at__actions">
          <a-button size="mini" @click="adjustUrl(group.url, 1800)">+30m</a-button>
          <a-button size="mini" class="btn-accent" @click="screenshotGroup(group.nodes)">截图</a-button>
          ...
          <a-button size="mini" @click="reloadUnrankedUrl(group.nodes)">刷新未上榜</a-button>
          <a-button size="mini" status="danger" @click="stopUrl(group.url)">全停</a-button>
        </a-space>
      </div>
    </div>
  </a-card>
