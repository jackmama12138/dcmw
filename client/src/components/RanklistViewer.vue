  ...
  <div class="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
    ...
    <div class="px-5 py-2.5 border-b border-gray-800 flex items-center gap-3">
      ...
      <input v-model="filterNickname" placeholder="按昵称搜索"
        class="bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5 text-xs focus:outline-none focus:border-indigo-500 w-36" />
    ...
    <!-- list header -->
    <div class="grid px-5 py-1.5 border-b border-gray-800 text-xs text-gray-600 select-none"
    ...
    <div class="overflow-y-auto" style="max-height:calc(100vh - 260px)">
      ...
      <div v-for="c in paged" :key="c._key"
        class="grid items-center px-5 py-2 border-b border-gray-800/40 last:border-0 hover:bg-gray-800/20 transition-colors text-xs"
        style="grid-template-columns:130px 70px 1fr 120px 50px 150px 50px">

        <span class="font-mono text-gray-400 truncate" :title="c.worker_id">{{ c.worker_id || '—' }}</span>
        <span class="font-mono text-indigo-300 truncate" :title="c.profile">{{ c.profile }}</span>

        <a :href="c.live_url" target="_blank"
          class="text-gray-400 truncate hover:text-indigo-300 transition-colors" :title="c.live_url">
          {{ shortUrl(c.live_url) }}
        </a>

        <span :class="c.is_logged_in ? 'text-gray-200' : 'text-gray-600'" class="truncate" :title="c.nickname">
          {{ c.nickname || (c.is_logged_in ? '—' : '未登录') }}
        </span>

        <div class="text-center">
          <span v-if="c.is_ranked"
            class="bg-amber-950 text-amber-400 border border-amber-800/50 px-1.5 py-0.5 rounded font-mono">
            {{ c.rank }}
          </span>
        ...
        <div class="text-right">
          <button @click="copy(c)"
            class="text-gray-600 hover:text-gray-300 transition-colors">复制</button>
        </div>
      </div>
...
async function load() {
  loading.value = true;
  try {
    allItems.value = await fetchRanklist();
  } catch {
    allItems.value = [];
  } finally {
    loading.value = false;
