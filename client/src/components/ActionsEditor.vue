  ...
  <div class="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
    <div class="px-5 py-3 border-b border-gray-800 flex items-center justify-between">
      ...
      <div class="flex items-center gap-3">
        <span v-if="hasCustom" class="text-xs text-indigo-400 bg-indigo-950 border border-indigo-800/50 px-2 py-0.5 rounded">自定义版本</span>
        <span v-else class="text-xs text-gray-600">内置默认版本</span>
        <button @click="resetDefault" v-if="hasCustom"
          class="text-xs text-gray-500 hover:text-gray-300 transition-colors border border-gray-700 rounded-lg px-2.5 py-1">
          恢复默认
        </button>
    ...
    <div class="p-5">
      ...
      <textarea v-model="code" spellcheck="false"
        class="w-full h-[600px] bg-gray-950 border border-gray-800 rounded-lg px-4 py-3 text-sm font-mono text-gray-200 focus:outline-none focus:border-indigo-600 resize-y"
